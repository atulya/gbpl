...
INNER JOIN tbl_left AS l ON l.id = s.left_id WHERE l.root_id = 3 AND d.status = 0 AND d.id = :data_id");
...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header"><?php if(isset($row_data)){ echo "Update"; }else{ echo "Add"; } ?> Hemera</h1>
        </div>
    </div><!--/.row-->
    <?php
    $row_count = "";
    $statement = $conn->prepare("SELECT `left_type_name`,`id` FROM `tbl_left` WHERE `root_id`=3");
    $statement->execute();
